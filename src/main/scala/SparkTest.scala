...
object SparkTest extends App {
  ...
  val df = sqlContext.read.
    ...
    option("header", "true").
    option("inferSchema", "true").load("employee.csv") //load data from a file

  val selectedCity = df.select("city")
  selectedCity.write.save("employeenew.csv")
  ...
  val usingSQL = sqlContext
    .sql("select * from my_table") //show all the csv file's data in temp table
  usingSQL.show()
}
